...
episode's transcript. Here's what it looks like:
...
sentiment. (We can also look for pockets where one person dominates the
conversation!)
...
It's become somewhat of a tradition for people to apologize to Chris Gethard's
mom, Sally, whenever they curse on the show. So before we move on from
sentiment, here's a related question we can try to answer: who owes Sally
Gethard a bigger apology, Chris or his callers?
...
get marked as profane. While this may indicate that the absolute counts in the
graph above are too high, the ratio between Gethard and his guests is probably
about right, since the profanity checker has a low bar regardless of who is
speaking.
...
1. Because of the small sample size (only ten episodes of transcripts), it's
...
So how can we do this? One way is to try to find _tells_ that will accurately
predict when Chris is talking as opposed to one of his callers.
...
able to correctly guess who said it (we can save the details of machine learning
for another time).

Of course, in our case, there's not really any guesswork involved: every line in
every transcript is clearly ascribed to either Gethard or the caller. But we can
still use this technique to extract features of the text that are more or less
likely to point to Gethard as the speaker.

I threw the kitchen sink at my computer in terms of features I thought might
have some predictive power. In the end, my kitchen sink approach yielded a
result that could accurately predict whether or not Gethard was the speaker of a
random line with about 70% accuracy. Here are the top twenty most informative
features for one of my training sessions (note that these features will change
if you teach the computer again, since the test set is a random subset of all of
the lines):
...
ask more questions than the callers do.
...
Well, I tried. I took all of Gethard's lines from the ten conversations, and
trained a [text-generating neural
network](https://github.com/minimaxir/textgenrnn) on them. Want to see how I did? Let's play a game.

<Quiz
  answerColors={[COLORS.DARK_BLUE, COLORS.PURPLE]}
  caption="Figure 9: Chris vs. Computer: who said it?"
  questionData={baQuizData}
  title="Who said it: Chris Gethard or a Machine?"
/>
...
callers, frames the conversation around them, and maintains and overall positive
sentiment. This is borne out in his conversational tells (his penchant for
asking questions, favoring "your" over "my", etc). Gethard frequently talks
about his show as giving a platform for everyday people, and the data backs up
this claim.

If you haven't listened to the show, I'd encourage you to give it a go. At its
best, it captures what it means to be human at this point in history. Each
episode is its own master class in listening and empathy.
