import NonlinearGamingRelationships from "./components/NonlinearGamingRelationships";
...
<Latex
  displayMode={true}
  str={`\\begin{aligned} A^{\\prime} (t) &= a \\times B(t) + c \\times A(t), \\\\ B^{\\prime} (t) &= b \\times A(t) + d \\times B(t), \\end{aligned}`}
/>
...
<Figure caption="On the left, our linear model from before; on the right, a new and nonlinear model.">
  <NonlinearGamingRelationships idx={0} />
</Figure>
...
<Latex
  displayMode={true}
  str={`\\begin{aligned} A^{\\prime} (t) &= a \\times B(t) ( 1 - \\left|B(t)\\right| ) + c \\times A(t), \\\\ B^{\\prime} (t) &= b \\times A(t) ( 1 - \\left|A(t)\\right| ) + d \\times B(t), \\end{aligned}`}
/>
...
<Sidebar>
  If you've gotten far enough in my book, you'll see that I talk about this
  nonlinear example at length. Unfortunately, I let a pretty egregious error
  slip through: the figures are correct, but the functions aren't! In the book I
  replaced the absolute value function by a different function accidentally. One
  thousand apologies!
</Sidebar>
...
<Sidebar direction="right">
  Technically, linear systems of differential equations can be chaotic, but only
  if they are infinitely dimensional. But that's a rabbit hole for another day.
</Sidebar>
...
<Figure caption="Chaotic relationship dynamics between three people.">
  <NonlinearGamingRelationships idx={1} />
</Figure>
...
<Latex
  displayMode={true}
  str={`\\begin{aligned} A^{\\prime} (t) &= a \\times B_{A}(t) ( 1 - \\left|B_{A}(t)\\right| ) + d \\times A(t), \\\\ B_{A}^{\\prime} (t) &= b \\times (A(t) - C(t)) ( 1 - \\left|A(t) - C(t)\\right| ) + e \\times B_{A}(t), \\\\ C^{\\prime} (t) &= c \\times B_{C}(t) ( 1 - \\left|B_{C}(t)\\right| ) + f \\times C(t), \\\\ B_{C}^{\\prime} (t) &= b \\times (C(t) - A(t)) ( 1 - \\left|C(t) - A(t)\\right| ) + e \\times B_{C}(t), \\\\ \\end{aligned}`}
/>
