...
const someNodesTouch = (nodes: HarassmentNode[], x: number, y: number, r: number) =>
  nodes.some((node) => node.x !== undefined && node.y !== undefined && (node.x - x) ** 2 + (node.y - y) ** 2 < (3 * r) ** 2);
...
const HarassmentNodeGroup: FC<HarassmentNodeGroupProps> = ({
  ...
  const simulationRef = useRef<Simulation<HarassmentNode, undefined> | null>(null);
  ...
  const propsRef = useRef({ blueOnBlueProb, greenOnGreenProb, blueOnGreenProb, greenOnBlueProb, playing, paused, handleShout });
  const shoutCountRef = useRef(shoutCount);

  useEffect(() => {
    propsRef.current = { blueOnBlueProb, greenOnGreenProb, blueOnGreenProb, greenOnBlueProb, playing, paused, handleShout };
  }, [blueOnBlueProb, greenOnGreenProb, blueOnGreenProb, greenOnBlueProb, playing, paused, handleShout]);
  ...
  useEffect(() => {
    ...
    const generateWave = (node: HarassmentNode) => {
      ...
      const soundWave = interval(() => {
        ...
        // @ts-ignore
        if (soundWave.__calledCount <= waveCount && propsRef.current.playing && !propsRef.current.paused) {
          // @ts-ignore
          ...
          select(gRef.current!)
            ...
            .classed("shout", true)
            .datum({ shoutCount: currentShoutCount, nodeKey: node.key, idx: currentIdx })
            .transition()
            .duration(2000)
            .ease(Math.sqrt)
            .attr("r", node.r * 8)
            ...
            .on("end", function(d: any) {
              ...
              const key = color === COLORS.BLUE ? "blueShoutsHeardFromBlueOnly" : "greenShoutsHeardFromGreenOnly";
    ...
    const handleCollision = (node1: HarassmentNode, node2: HarassmentNode) => {
      if (propsRef.current.playing && !propsRef.current.paused) {
        const { blueOnBlueProb, greenOnGreenProb, blueOnGreenProb, greenOnBlueProb } = propsRef.current;
        const probabilities: Record<string, [number, number]> = {
          [`${COLORS.BLUE}:${COLORS.BLUE}`]: [blueOnBlueProb, blueOnBlueProb],
          [`${COLORS.BLUE}:${COLORS.GREEN}`]: [blueOnGreenProb, greenOnBlueProb],
          [`${COLORS.GREEN}:${COLORS.BLUE}`]: [greenOnBlueProb, blueOnGreenProb],
          [`${COLORS.GREEN}:${COLORS.GREEN}`]: [greenOnGreenProb, greenOnGreenProb],
        };
        const nodeColors = [node1.properties.color, node2.properties.color].join(":");
    ...
    const sim = forceSimulation<HarassmentNode>()
      ...
      .force(
        "bounce", 
        forceBounce<HarassmentNode>().radius((node: HarassmentNode) => node.r).onImpact(handleCollision)
      )
      .force(
        "surface",
  ...
  useEffect(() => {
    ...
    const checkIntersections = () => {
      ...
      const nodeSelection = gSelection.selectAll<SVGCircleElement, HarassmentNode>(".node");
      
      gSelection.selectAll<SVGCircleElement, {shoutCount: number, nodeKey: string}>(".shout")
        .each(function(d) {
          if (!d) return;
          const color = String(d.nodeKey).split("-")[0];
          ...
          nodeSelection.each(function(nodeData) {
            ...
            if (nodeColor !== color && distance < r + waveR) {
              ...
              const key = nodeColor === COLORS.BLUE ? "blueShoutsHeardFromGreen" : "greenShoutsHeardFromBlue";
              propsRef.current.handleShout(key, d.shoutCount);
            }
          });
        });
    ...
    const tick = () => {
      ...
      const enterNodes = nodesSel.enter()
        .append("circle")
        .classed("node", true)
        .attr("r", d => d.r)
    ...
    const blueArr: HarassmentNode[] = Array.from({ length: blueCount }, (_, i) => ({
      key: `${colorBlue}-${i}`,
      properties: { color: colorBlue },
      r: 15
    }));
    
    const greenArr: HarassmentNode[] = Array.from({ length: greenCount }, (_, i) => ({
      key: `${colorGreen}-${i}`,
  ...
  return (
    <g>
      <rect x={0} y={0} width={width} height={height} stroke="#d1d5db" strokeWidth={3} fill="none" />
      <g ref={gRef} />
    </g>
  );
};
