...
describe("EfficiencyGapTable", () => {
  ...
  it("renders the table when districtCounts is provided", () => {
    ...
    const districtCounts: [number, number][] = Array.from({ length: 6 }, () => [5, 4]);
    render(<EfficiencyGapTable districtCounts={districtCounts} />);
    expect(screen.getByText(/sample efficiency gap calculation/i)).toBeInTheDocument();
  });
  ...
  it("shows one row per district in the table", () => {
    ...
    const { container } = render(<EfficiencyGapTable districtCounts={districtCounts} />);
