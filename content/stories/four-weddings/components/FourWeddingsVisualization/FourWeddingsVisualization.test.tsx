...
vi.mock("../SelectableUSMap", () => ({
  default: () => <div data-testid="mock-map" />,
}));
vi.mock("../SelectableHistogram", () => ({
  default: () => <div data-testid="mock-histogram" />,
}));
vi.mock("../SelectablePieChart", () => ({
  default: () => <div data-testid="mock-pie" />,
}));
vi.mock("../SelectableScatterplot", () => ({
  default: () => <div data-testid="mock-scatter" />,
}));

describe("FourWeddingsVisualization Dispatcher", () => {
  it('renders the US Map when vizType is "map"', () => {
    const { getByTestId } = render(
      <FourWeddingsVisualization vizType="map" vizIndex={1} />,
    );
  ...
  it('renders the Histogram when vizType is "histogram"', () => {
    const { getByTestId } = render(
      <FourWeddingsVisualization vizType="histogram" vizIndex={1} />,
    );
  ...
  it('renders the Pie Chart when vizType is "pie"', () => {
    const { getByTestId } = render(
      <FourWeddingsVisualization vizType="pie" vizIndex={1} />,
    );
  ...
  it('renders the Scatterplot when vizType is "scatter"', () => {
    const { getByTestId } = render(
      <FourWeddingsVisualization vizType="scatter" vizIndex={1} />,
    );
