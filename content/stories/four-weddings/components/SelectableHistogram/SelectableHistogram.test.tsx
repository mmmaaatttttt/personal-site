...
type BarDatum = { height: number; x0: number; x1: number };
...
describe("SelectableHistogram Component", () => {
  ...
  const mockOptions: HistogramOption[] = [
    {
      value: "v1",
      label: "Option 1",
      accessor: (d: WeddingData) => (d as unknown as { value: number }).value,
      step: 10,
      format: ".0f",
    },
    {
      ...
      accessor: (d: WeddingData) =>
        (d as unknown as { value: number }).value * 2,
      step: 50,
      format: ".0f",
    },
  ];
  ...
  it("bins data correctly for the initial selection", () => {
    const { getByTestId } = render(
      <SelectableHistogram data={mockData} selectOptions={mockOptions} />,
    );
    ...
    const countWithHeight = barData.filter((b) => b.height > 0);
    expect(countWithHeight).toHaveLength(3);

    const bin10to20 = barData.find((b) => b.x0 === 10);
  ...
  it("maintains consistent bin widths for all bars", () => {
    ...
    const validBins = barData.filter(
      (b) => b.x0 !== undefined && b.x1 !== undefined,
    );
