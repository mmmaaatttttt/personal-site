...
If you'd like to try calculating any of this own your own, be my guest. Don't scroll any further until you're ready to see the results. For the rest of you, here's a table with every possible postiive payout along with its likelihood when you do a fresh pull.

<Figure caption="Payouts and probabilities for each result with a positive payout.">
  <PayoutTable data={payoutRows} />
</Figure>

Note that these are all mutually exclusive by construction. For example, the probability of getting three single coins is listed as 6.13%. This is the probability of getting exactly this outcome with a payout of 3; in other words, the fourth sybmol must be a dash, crown, money bag, or net. The other symbols would all impact the payout and are therefore represented in other rows.
...
I definitely spent more time than this. So maybe my strategy was bad. Though if I'm being kinder to myself, the distribution of payouts is also pretty clumpy. There are a lot of small losses, but the occasional 🍀 or jackpot can trigger substantial payouts when it comes up.
...
One way to model this is introduce a sliding scale that roughly maps to how good your strategy is. At one end of the scale, you play the game perfectly; this is the model we've been using the entire time. At the other end of the scale, the strategy is just random choice (i.e. not a very good strategy). 

In between, without getting too lost in the mathematical weeds, the slider controls how likely we are to choose the optimal value vs making an incorrect choice.The options are weighedd such that two good options will be roughly equally likely to be chosen, while a horrible option will have a low (but nonzero) likelidhood of selection.
...
I'll spare you the mathematical details. If you're curious to learn more about the underlying models here, this is a [discrete choice model](https://en.wikipedia.org/wiki/Discrete_choice). More specifically, it's a conditional logit model, where our slider parameter roughly controls how likely a player is to choose the best alternative over the other available options. 
...
Still, the fact remains: either one of these machines essentially offer an infinite money machine in Blue Prince. The only question is how much you value your time, because these machines dispense that infinite money at a pace that would make any pit boss proud.
