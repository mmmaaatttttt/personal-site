...
interface MultiBarGraphProps {
  ...
  getTooltipData: (d: MultiBarData) => { title: string; body: string[] | string };
  width?: number;
  yAxisLabel?: string;
  yMax?: number;
}
...
const MultiBarGraph: React.FC<MultiBarGraphProps> = ({
  ...
  const stackData = useMemo(() => {
    try {
      return d3stack().keys(labels).offset(stackOffsetNone)(data.map((d) => d.counts || {}));
    } catch (e) {
  ...
  const tooltipData = useMemo(() => data.map(getTooltipData), [data, getTooltipData]);
  
  const yMin = min(stackData[0], (d) => d[0]) || 0;
  const computedYMax = yMax || max(stackData[stackData.length - 1], (d) => d[1]) || 1;
  ...
  return (
    <NarrowContainer width={containerWidth.toString()} className="w-full relative">
      <Legend
        title={legendTitle}
          ...
          <g clipPath={`url(#clip-path-${id})`}>
            <AnimatePresence>
              {data.map((_, colIndex) => {
                ...
                return (
                  <g key={`col-${colIndex}`} className="opacity-80 hover:opacity-100 transition-opacity cursor-pointer">
                    {extentsForCol.map(([minVal, maxVal], layerIndex) => {
                      const rectY = yScale(maxVal);
                      const rectHeight = Math.max(0, yScale(minVal) - yScale(maxVal));
                      ...
                      return (
                        <motion.rect
                          ...
                          animate={{ x: colX, y: rectY, height: rectHeight, width: (xScale.step() || 0) * 0.8 }}
                          transition={{ duration: 0.5, delay: colIndex * 0.02 + layerIndex * 0.1 }}
