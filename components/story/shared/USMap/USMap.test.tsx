...
describe('USMap Component', () => {
  ...
  it('applies the correct fill based on data and color scale', () => {
    const { getAllByTestId } = render(<USMap {...defaultProps} domain={[10, 20]} />);
    const states = getAllByTestId('us-state');
    
    const alabama = states.find(s => s.getAttribute('data-title') === 'Alabama');
    const alaska = states.find(s => s.getAttribute('data-title') === 'Alaska');
    const wyoming = states.find(s => s.getAttribute('data-title') === 'Wyoming'); // No data

    // Linear scale [10, 20] -> ['#ffffff', '#000000']
    // 10 -> #ffffff
    // 20 -> #000000
    expect(alabama?.getAttribute('fill')).toMatch(/#ffffff|rgb\(255, 255, 255\)/i);
    expect(alaska?.getAttribute('fill')).toMatch(/#000000|rgb\(0, 0, 0\)/i);
    expect(wyoming?.getAttribute('fill')).toMatch(/#eeeeee|rgb\(238, 238, 238\)/i);
  });
});
