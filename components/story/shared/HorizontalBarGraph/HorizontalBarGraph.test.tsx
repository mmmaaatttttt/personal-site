...
describe('HorizontalBarGraph Component', () => {
  ...
  it('renders correctly with given data', () => {
    ...
    const bars = Array.from(container.querySelectorAll('rect')).filter(r => r.getAttribute('fill') && r.getAttribute('fill') !== 'none');
  ...
  it('handles empty data gracefully', () => {
    ...
    const { container } = render(<HorizontalBarGraph {...defaultProps} data={[]} />);
    const bars = Array.from(container.querySelectorAll('rect')).filter(r => r.getAttribute('fill') && r.getAttribute('fill') !== 'none');
