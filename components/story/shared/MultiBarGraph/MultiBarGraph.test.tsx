...
describe('MultiBarGraph Component', () => {
  ...
  it('renders the correct number of stacked bar groups', () => {
    ...
    const getBars = (c: HTMLElement) => Array.from(c.querySelectorAll('rect')).filter(r => r.getAttribute('fill'));
  ...
  it('respects the yMax prop if provided', () => {
    ...
    const initialHeight = parseFloat(getBars(container)[0].getAttribute('height') || '0');
    
    rerender(<MultiBarGraph {...defaultProps} yMax={1000} />);
    const largeYMaxHeight = parseFloat(getBars(container)[0].getAttribute('height') || '0');
