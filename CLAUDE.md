...
| `SliderGroup`                                                                            | Renders an array of labeled sliders; takes `data: SliderData[]`. Use with `useSliders` from `hooks/useSliders.ts` — the hook manages state and returns `{ values, sliderData }` where `sliderData` is passed directly to `SliderGroup`.                                                                                                           |
| `HorizontalBar`                                                                          | Animated proportional bar chart; takes `data: { size, color, tooltipText }[]`                                                                                                                                                                                                                                                                    |
| `Legend`, `ColoredSpan`                                                                  | Simple presentational                                                                                                                                                                                                                                                                                                                            |
| `Figure`                                                                                 | Wrapper for all interactives in MDX. Props: `caption?`, `className?`, `captionMarginTop?`, `bleed?` (default `true`). Applies breakout (up to `w-[130%]` at large viewports) unless `bleed={false}`. Renders an expand/collapse button. Caption shows "Figure N: text" when wrapped in `FigureProvider`. Use `<Figure caption="..."><YourComponent /></Figure>` — do not add `caption` props to interactive components themselves. |
| `GamingRelationships`                                                                    | ODE-based relationship visualizer; takes `visData` (includes diff eq functions — not serializable), `min/max/step`. Use a story-specific `"use client"` wrapper to expose only `idx` to MDX.                                                                                                                                                     |
...
### NarrowContainer widths inside Figure (breakout scaling)

`Figure` always applies the breakout (up to `w-[130%]` at large viewports) unless `bleed={false}`. `NarrowContainer` and `SliderProvider` widths are percentages of the Figure container, **not** the prose column. Size them for the breakout width, not for prose width.
...
If the interactives look too wide inside a Figure, **do not pass `bleed={false}`** — instead reduce the internal NarrowContainer widths by ÷1.3. That was tried and reverted; the global behavior is intentional.
